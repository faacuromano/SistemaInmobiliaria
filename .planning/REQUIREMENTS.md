...
- [x] **FIN-05**: `calculateInstallmentPreview()` parity test — same inputs produce identical outputs as `generateInstallments()`
- [x] **FIN-06**: `recalculateInstallments()` tested for first refuerzo — unpaid installments reduced, `originalAmount` set
- [x] **FIN-07**: `recalculateInstallments()` tested for second refuerzo — `originalAmount` NOT overwritten
- [x] **FIN-08**: `recalculateInstallments()` tested for edge case: reduction > installment amount → clamped to 0
...
- [x] **GRID-03**: Clicking a lot opens detail panel showing buyer, price, area, tags, and sale link
- [x] **GRID-04**: Mobile: lot detail opens as Sheet drawer
- [x] **GRID-05**: View mode persists across navigation via localStorage
- [x] **GRID-06**: Print view via `@media print` — hides controls, renders lots flat for client meetings
