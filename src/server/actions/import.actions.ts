...
const saleImportSchema = z.object({
  ...
  firstInstallmentMonth: z.string().optional().nullable(), // "YYYY-MM"
  collectionDay: z.number().int().min(1).max(31).optional().nullable(),
  downPayment: z.number().optional().nullable(),
  status: z.enum(["ACTIVA", "CANCELADA", "COMPLETADA", "CONTADO", "CESION"]).default("ACTIVA"),
  paymentWindow: z.string().optional().nullable(),
  notes: z.string().optional().nullable(),
  saleDate: z.string().optional().nullable(), // "YYYY-MM-DD"
  groupId: z.string().optional().nullable(), // Para multi-lote
});
...
function parseInputData(
  ...
  if (format === "csv") {
    ...
    if (result.errors.length > 0 && result.data.length === 0) {
      return { rows: [], error: `CSV invalido: ${result.errors[0].message}` };
    }
    return { rows: coerceNumericFields(result.data as Record<string, string>[]) };
  }
...
/** CSV values are all strings — coerce numeric-looking fields for Zod validation */
function coerceNumericFields(rows: Record<string, string>[]): unknown[] {
  const numericKeys = [
    "totalPrice", "totalInstallments", "regularInstallmentAmount",
    "firstInstallmentAmount", "collectionDay", "downPayment",
  ];
  return rows.map((row) => {
...
export async function importPersons(
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      // Check for existing person by DNI (skip if exists)
      if (data.dni) {
...
export async function importSales(
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      // Check if lot already has a sale (skip duplicates)
      ...
      if (
        ...
        if (data.firstInstallmentAmount && data.totalInstallments > 1) {
          ...
          regularAmount =
            Math.round((remaining / (data.totalInstallments - 1)) * 100) / 100;
        } else {
          regularAmount =
      ...
      await prisma.$transaction(async (tx) => {
        const sale = await tx.sale.create({
          data: {
            ...
            status: data.status as "ACTIVA" | "CANCELADA" | "COMPLETADA" | "CONTADO" | "CESION",
            notes: data.notes || null,
        ...
        if (
          ...
          const installments = generateInstallments({
            ...
            firstInstallmentAmount: data.firstInstallmentAmount ?? undefined,
