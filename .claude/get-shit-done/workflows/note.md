...
- **Global scope**: `D:/Proyectos/sistemaInmobiliaria/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Proyectos/sistemaInmobiliaria/.claude/notes/`)
...
2. Glob `D:/Proyectos/sistemaInmobiliaria/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Proyectos/sistemaInmobiliaria/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Proyectos/sistemaInmobiliaria/.claude/notes/` — works in any directory
